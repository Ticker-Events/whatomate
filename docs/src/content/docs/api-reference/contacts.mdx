...
```json
{
  ...
  "data": {
    ...
    "last_message_at": "2024-01-01T12:00:00Z",
    "last_inbound_at": "2024-01-01T12:00:00Z",
    "is_closed": false,
    "awaiting_reply_since": "2024-01-01T12:00:00Z",
    "created_at": "2024-01-01T00:00:00Z"
  }
}
```
...
`is_closed` is optional. Setting it only toggles whether the conversation is closed; it does **not** start or clear `awaiting_reply_since`.

### First-response wait

Each contact tracks whether the conversation is waiting for a store reply:

| Event | `is_closed` | `awaiting_reply_since` |
| --- | --- | --- |
| Inbound message | `false` (reopen) | Set if empty **or** the chat was closed; otherwise keep |
| Outbound message (agent or bot) | `true` (auto-close) | Cleared |
| Manual close (`is_closed: true`) | `true` | Left in place so reopen can resume the wait |
| Manual reopen (`is_closed: false`) | `false` | Unchanged |

When Firestore is configured (`[firebase].credentials`), these fields are projected onto `contacts/{id}` as camelCase `isClosed` and `awaitingReplySince` (the clock field is deleted when empty) so the tiqr.store inbox can read them.
