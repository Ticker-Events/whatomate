...
When storefront (commerce) checkout asks for a delivery address:

- If the **store country is India** (`IN`) **and** the customer's WhatsApp number is Indian (`91…`), WhatsApp's interactive address form is sent (`interactive.type` = `native_flow`, `action.name` = `address_message`). The customer fills name, phone, pin code, street, city, and state in WhatsApp, and the reply is mapped into the order's `new_address`.
- For **any other store country**, a **non-India WhatsApp number**, or if the store country cannot be loaded, the bot asks for the address as a normal text message.

## Commerce MCP security

The commerce MCP API key is write-only. Settings responses expose only
`ai_commerce_mcp_api_key_set`; neither the MCP key nor the AI provider key is
returned to the browser or written to audit-log snapshots. Both values are
encrypted at rest using `app.encryption_key`.

Commerce also stores an optional **REST Endpoint URL** (`ai_commerce_rest_url`)
used by TiQR Store flow nodes set to REST. It is the ticker-events API origin
(no `/mcp` path); buyer routes under `/service/buyer/` are appended by Whatomate.

Production requires `app.encryption_key` to be at least 32 characters. Run the
server once with `--migrate` after deploying this change to encrypt any legacy
plaintext values in `chatbot_settings`. The migration is transactional and
idempotent.

For shared-key rotation:

1. Save the new Backend `MCP_API_KEY` in the chatbot commerce settings.
2. Replace the Backend environment secret and restart its MCP process.
3. Restart Whatomate workers and verify a store lookup.
4. Revoke the old value from the deployment secret store.
- If WhatsApp rejects or cannot show the form (for example **1026 Receiver Incapable**, or send failures such as **131009**), checkout falls back to the text prompt. Customers can also type an address even when the form was sent.

This form is only available for India businesses and India customers on WhatsApp.
...
From the Conversation Flows list you can:

- **Export** a flow as a JSON file (`whatomate.chatbot_flow` format)
- **Import** a previously exported JSON file as a new flow
- **Duplicate** a flow in the same organization

Imported and duplicated flows are created **disabled**. Cross-reference
IDs inside the graph (other flows, teams, WhatsApp Flows) are preserved
as-is — fix them in the builder if you restore into a different org.
...
| `tiqr_store_api` | Call a TiQR storefront operation (collections, products, guest checkout, orders) via MCP or buyer REST. Store ID and URLs come from AI settings → Commerce. | `http:2xx`, `http:non2xx` |
...
`tiqr_store_api` is a first-party clone of `api_call` for TiQR storefront data. Authors pick an **API type** (`MCP` or `REST`) and an operation; the node injects **Store ID** from **AI settings → Commerce**. Endpoints are not authored on the node.

- **MCP** uses Commerce MCP URL + optional MCP API key (same credentials as AI commerce tools).
- **REST** uses Commerce REST Endpoint URL (ticker-events origin; paths under `/service/buyer/` are appended).

Supported operations: list/search collections, list/search products, product details, product options, store details, store info, FAQs, check delivery, guest checkout (`create_order`), get order, lookup order status, retry payment.

Check delivery, lookup order status, and retry payment are **MCP-only** (no guest-accessible buyer REST equivalent). Selecting REST hides those operations in the dropdown.

Checkout is guest `create_order` (phone is taken from the WhatsApp session when omitted). Logged-in JWT cart (`get_cart` / `upsert_cart` / `checkout_cart`) is not available from a flow session.

Map response fields with the same JSON-path syntax as `api_call` (`products[0].name`). Missing commerce settings for the selected transport, or a tool/HTTP error, routes to `http:non2xx`.
...
`prompt.store_as`, `api_call.response_mapping`, `tiqr_store_api.response_mapping`, or earlier inputs.
...
Firestore `contacts/{id}` is updated on transfer create, resume, and SLA expire:

- `chatbotPaused` — `true` while an agent transfer is active
- `activeTransferId` — the active transfer UUID; deleted when the chatbot resumes

Existing transfers created before this field existed will not show paused state until a new pause, resume, or expire writes the document.
