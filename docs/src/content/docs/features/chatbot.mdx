...
| `tiqr_store_api` | Call a TiQR storefront operation (collections, products, guest checkout, orders). Store ID and MCP credentials come from AI settings → Commerce. | `http:2xx`, `http:non2xx` |
...
`tiqr_store_api` is a first-party clone of `api_call` for TiQR storefront data. Authors pick an operation from a dropdown; the node injects **Store ID**, MCP URL, and MCP API key from **AI settings → Commerce**. Endpoints are not authored on the node.
...
Map response fields with the same JSON-path syntax as `api_call` (`products[0].name`). Missing commerce settings or a tool error routes to `http:non2xx`.
