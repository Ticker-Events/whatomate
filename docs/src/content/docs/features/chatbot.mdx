...
- If the **store country is India** (`IN`) **and** the customer's WhatsApp number is Indian (`91…`), WhatsApp's interactive `address_message` form is sent. The customer fills name, phone, pin code, street, city, and state in WhatsApp, and the reply is mapped into the order's `new_address`.
...
- If WhatsApp cannot show the form (error **1026 Receiver Incapable**), checkout falls back to the text prompt. Customers can also type an address even when the form was sent.
